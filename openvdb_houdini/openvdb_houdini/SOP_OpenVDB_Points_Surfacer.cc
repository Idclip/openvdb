...
SOP_OpenVDB_Points_Surfacer::cookVDBSop(OP_Context& context)
{
    ...
    try {
        ...
        const float radiusScale = evalFloat("particleradius", 0, time);
        ...
        for (; vdbIt; ++vdbIt) {
            ...
            const double searchRadius = (absoluteInfluence ?
                influenceRadius : (influenceScale * radiusScale * averageRadius));
            ...
            else if (mode == 2)
            {
                ...
                if (!disableSurface) {
                    ...
                    es.pca = a;
                    es.interrupter = &boss;
                    es.sphereScale = dropletScale;
                    es.radiusScale = radiusScale;
                    es.radius = radiusAttributeName;
                    es.halfband = halfBand;

                    std::cerr << "es.sphereScale " << es.sphereScale << std::endl;
