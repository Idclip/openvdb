...
auto usage_execute(const bool verbose)
{
    return [=](std::ostream& os) -> std::ostream& {
        ...
        if (verbose) {
            ...
            "\n" <<
            "    Grids can also be piped in/out from cout, allowing for invocations of this\n" <<
            "    binary to be chained together:\n" <<
            "\n" <<
            "         " << gProgName << " density.vdb -s \"@density += 1;\" |  // pipe into next command\n" <<
            "           " << gProgName << " fog.vdb -s \"@density += @fog;\" -o out.vdb\n" <<
            "\n" <<
...
main(int argc, char *argv[])
{
    ...
    // If stdin/stdout are not tty's then we're receiving data through pipes
    const bool pipein =  !isatty(fileno(stdin));
    const bool pipeout = !isatty(fileno(stdout));
